...
`player_items` stores the current Item balance. `item_transactions` is the
append-only audit trail generated by a database trigger for every non-zero
insert, update, or delete delta.

```text
item_transaction_id   PK
player_id             FK
item_type
amount                signed integer
transaction_type      BASELINE | GRANT | CONSUME | DELETE
balance_after
created_at
```

`item_transactions`, `player_xp_transactions`, `card_ownership_history`, and
`security_events` reject UPDATE and DELETE. Historical corrections must be new
compensating records, never mutations of existing audit rows.
